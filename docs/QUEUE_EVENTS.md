...
3. **DM sent to each player** with:
   ...
   - Google Apps Script web app link for replay submission
...
- Sends DM with embed
- Generates Google Apps Script web app URL
...
The replay URL is generated by `UrlGenerator.generateWebAppUrl()` using:
- `APPSCRIPT_BASE_URL`

The bot sends links in the form:

```text
https://script.google.com/macros/s/.../exec?id=SCRIM-ABC123&players=...&maps=...&timestamp=...
```
